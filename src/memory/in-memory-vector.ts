...
/**
 * Internal entry stored in the in-memory vector store.
 */
 ...
 * import { embed } from "./my-embedder";
 *
 * const store = new InMemoryVectorStore<string>({ embed });
 *
...
export class InMemoryVectorStore<T = unknown> implements VectorMemory<T> {
  private readonly store = new Map<string, VectorEntry<T>>();
