...
- **How do I tune token budgets?** Pass a `budget` plus a tokenizer to `render()` and adjust priorities on regions. Providers (`OpenAIProvider`, `AnthropicProvider`, `AISDKProvider`) supply a default tokenizer if you don't pass one—see `docs/tokenization.md` for accuracy vs. convenience tradeoffs.
